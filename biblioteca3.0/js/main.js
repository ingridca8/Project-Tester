...
async function fetchBooks(tableName) {
    try {
        if (!supabaseClient) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (hamburger && navLinks) {
        hamburger.addEventListener('click', function(e) {
            ...
            console.log('Menú toggled:', navLinks.classList.contains('open'));
        });
    }
...
// ========================================
// 5. FUNCIONES DE MODAL (COMPARTIDAS)
// ========================================
...
async function initStats() {
    ...
    const no33 = await fetchBooks('no_33');
    const wishlist = await fetchBooks('wishlist');
...
async function initBiblioteca() {
    ...
    bibliotecaBooks = await fetchBooks('books');
    ...
    if (books.length === 0) {
        container.innerHTML = `
            <div style="text-align:center;padding:3rem;color:var(--text-muted);grid-column:1/-1;">
                <p style="font-size:1.2rem;">📚 No hay libros</p>
                <p style="font-size:0.9rem;">Haz clic en + para agregar</p>
            </div>
        `;
        return;
    }
...
function openBibliotecaEdit(book) {
    ...
    document.getElementById('deleteBookBtn').dataset.id = book.id;
    openModal('modalOverlay');
}
...
async function saveBibliotecaBook() {
    ...
    try {
        if (id) {
            await supabaseClient.from('books').update(data).eq('id', id);
        } else {
...
async function initNo33() {
    ...
    no33Books = await fetchBooks('no_33');
...
async function toggleNo33Status(id) {
    ...
    try {
        await supabaseClient.from('no_33').update({ status: next }).eq('id', id);
        no33Books = await fetchBooks('no_33');
        renderNo33(no33Books);
        populateGenreFilter('genreFilterNo33', no33Books);
    } catch (e) {
...
function openNo33Add() {
    ...
    document.getElementById('deleteBookBtnNo33').style.display = 'none';
...
async function saveNo33Book() {
    ...
    try {
        if (id) {
            await supabaseClient.from('no_33').update(data).eq('id', id);
        } else {
            await supabaseClient.from('no_33').insert([data]);
        }
...
async function deleteNo33Book() {
    ...
    if (!id || !confirm('¿Eliminar este libro?')) return;
    try {
        await supabaseClient.from('no_33').delete().eq('id', id);
        closeModal('modalOverlayNo33');
        no33Books = await fetchBooks('no_33');
        renderNo33(no33Books);
        populateGenreFilter('genreFilterNo33', no33Books);
...
async function initWishlist() {
    ...
    wishlistBooks = await fetchBooks('wishlist');
...
function setupWishlistEvents() {
    ...
    // Poblar años
    const years = [...new Set(books => books.map(b => b.year).filter(Boolean))].sort((a, b) => b - a);
...
function openWishlistAdd() {
    ...
    document.getElementById('deleteBookBtnWishlist').style.display = 'none';
...
async function saveWishlistBook() {
    ...
    try {
        if (id) {
            await supabaseClient.from('wishlist').update(data).eq('id', id);
        } else {
            await supabaseClient.from('wishlist').insert([data]);
        }
